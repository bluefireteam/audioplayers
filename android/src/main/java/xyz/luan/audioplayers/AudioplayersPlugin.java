...
public class AudioplayersPlugin implements MethodCallHandler, FlutterPlugin, ActivityAware {
	...
	public void onAttachedToActivity(ActivityPluginBinding binding) {
		// clientHandler = new
		// ClientHandler(flutterPluginBinding.getFlutterEngine().getDartExecutor());
	...
	public MediaControllerCompat.Callback controllerCallback = new MediaControllerCompat.Callback() {
		...
		public void onPlaybackStateChanged(PlaybackStateCompat state) {
			...
			// invokeMethod("onPlaybackStateChanged", state.getState(), state.getActions(),
			// state.getPosition(), state.getPlaybackSpeed(), updateTimeSinceEpoch);
		}
		...
		switch (call.method) {
			case "startHeadlessService": {
				...
				AudioService.init(activity, resumeOnClick, androidNotificationChannelName,
						...
						androidNotificationClickStartsActivity, androidNotificationOngoing,
						androidStopForegroundOnPause, androidStopOnRemoveTask, artDownscaleSize, backgroundHandler);
				...
				if (mediaBrowser == null) {
					...
					mediaBrowser = new MediaBrowserCompat(context, new ComponentName(context, AudioService.class),
			...
			case "setNotification": {
				...
				final int elapsedTime = call.argument("elapsedTime");
				...
				player.setNotification(title, albumTitle, artist, imageUrl, maxDuration, elapsedTime);
				break;
			}
			case "getDuration": {
	...
	private static class BackgroundHandler implements MethodCallHandler, AudioService.ServiceListener {
				...
				backgroundHandler.backgroundChannel.invokeMethod("onLoadChildren", list, new MethodChannel.Result() {
					...
					public void success(Object obj) {
						...
						for (Map<?, ?> rawMediaItem : rawMediaItems) {
							...
							// mediaItems.add(new
							// MediaBrowserCompat.MediaItem(mediaMetadata.getDescription(),
		...
		public void onSetRating(RatingCompat rating, Bundle extras) {
			// invokeMethod("onSetRating", rating2raw(rating),
			// extras.getSerializable("extrasMap"));
		}
		...
		public void onMethodCall(MethodCall call, Result result) {
			...
			switch (call.method) {
				...
				case "setState":
					...
					// On the flutter side, we represent the update time relative to the epoch.
					// On the native side, we must represent the update time relative to the boot
					// time.
					...
					for (Map<?, ?> rawControl : rawControls) {
						...
						actions.add(
								AudioService.instance.action(resource, (String) rawControl.get("label"), actionCode));
					}
					...
					if (compactActionIndexList != null) {
						compactActionIndices = new int[Math.min(AudioService.MAX_COMPACT_ACTIONS,
								compactActionIndexList.size())];
						for (int i = 0; i < compactActionIndices.length; i++)
							compactActionIndices[i] = (Integer) compactActionIndexList.get(i);
					}
					AudioService.instance.setState(actions, actionBits, compactActionIndices, playbackState, position,
							speed);
					result.success(true);
					break;
				case "stopped":
				...
				case "androidForceEnableMediaButtons":
					...
					if (silenceAudioTrack == null) {
						...
						silenceAudioTrack = new AudioTrack(AudioManager.STREAM_MUSIC, SILENCE_SAMPLE_RATE,
								AudioFormat.CHANNEL_CONFIGURATION_MONO, AudioFormat.ENCODING_PCM_8BIT, silence.length,
	...
	private static Map<?, ?> mediaMetadata2raw(MediaMetadataCompat mediaMetadata) {
		...
		if (mediaMetadata.containsKey(MediaMetadataCompat.METADATA_KEY_DISPLAY_SUBTITLE))
			raw.put("displaySubtitle",
					mediaMetadata.getText(MediaMetadataCompat.METADATA_KEY_DISPLAY_SUBTITLE).toString());
		if (mediaMetadata.containsKey(MediaMetadataCompat.METADATA_KEY_DISPLAY_DESCRIPTION))
			raw.put("displayDescription",
					mediaMetadata.getText(MediaMetadataCompat.METADATA_KEY_DISPLAY_DESCRIPTION).toString());
		if (mediaMetadata.containsKey(MediaMetadataCompat.METADATA_KEY_RATING)) {
			// raw.put("rating",
			// rating2raw(mediaMetadata.getRating(MediaMetadataCompat.METADATA_KEY_RATING)));
		}
