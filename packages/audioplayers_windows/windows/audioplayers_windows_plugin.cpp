...
class AudioplayersWindowsPlugin : public Plugin {
 ...
 private:
  ...
  std::map<std::string, std::unique_ptr<EventChannel<EncodableValue>>> playerEventChannels;
...
void AudioplayersWindowsPlugin::CreatePlayer(std::string playerId) {
  ...
  // Keep the event channel alive as long as the plugin/player exists
...
void AudioplayersWindowsPluginRegisterWithRegistrar(
  ...
  if (!registrar) {
      std::cerr << "====== Audioplayers: Error - C-API Registrar is NULL!" << std::endl;
      std::cerr.flush();
      return;
  }

  auto* plugin_registrar = PluginRegistrarManager::GetInstance()
          ->GetRegistrar<PluginRegistrarWindows>(registrar);
  
  if (plugin_registrar) {
      std::cout << "====== Audioplayers: PluginRegistrarWindows acquired" << std::endl;
      std::cout.flush();
      AudioplayersWindowsPlugin::RegisterWithRegistrar(plugin_registrar);
  } else {
      std::cerr << "====== Audioplayers: Failed to get plugin registrar from manager" << std::endl;
      std::cerr.flush();
  }
}
