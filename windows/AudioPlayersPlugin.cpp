...
// static
void AudioPlayersPlugin::RegisterWithRegistrar(flutter::PluginRegistrar *registrar) {
  auto channel =
      std::make_unique<flutter::MethodChannel<flutter::EncodableValue>>(
          registrar->messenger(), "AudioPlayersPlugin",
          &flutter::StandardMethodCodec::GetInstance());
  auto *channel_pointer = channel.get();

  auto plugin = std::make_unique<AudioPlayersPlugin>(std::move(channel));
...
void AudioPlayersPlugin::HandleMethodCall(
  ...
  if (method_call.method_name().compare("getPlatformVersion") == 0) {
    ...
    auto url = method_call.args.FindMember("url");
